...
use crate::{
    ...
    state::{
        rbac::Roles,
        storage::{RBAC_PERMISSIONS, TIMELOCK_DELAY},
    },
    ContractError,
};
...
pub fn grant_role(
    ...
    let mut current_roles = RBAC_PERMISSIONS
...
mod test {
    ...
    fn test_can_invoke_add_path() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::AddRateLimit].into_iter().collect(),
            )
            .unwrap();

        assert!(can_invoke_message(&deps.as_mut(), &info_foobar, &msg).is_ok());
        assert!(can_invoke_message(&deps.as_mut(), &info_foobarbaz, &msg).is_err());
    }
    ...
    fn test_can_invoke_remove_path() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::RemoveRateLimit].into_iter().collect(),
            )
            .unwrap();
    ...
    fn test_can_invoke_reset_path_quota() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::ResetPathQuota].into_iter().collect(),
            )
            .unwrap();
    ...
    fn test_can_invoke_grant_role() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::GrantRole].into_iter().collect(),
            )
            .unwrap();
    ...
    fn test_can_invoke_revoke_role() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::RevokeRole].into_iter().collect(),
            )
            .unwrap();
        assert!(can_invoke_message(&deps.as_mut(), &info_foobar, &msg).is_ok());
        assert!(can_invoke_message(&deps.as_mut(), &info_foobarbaz, &msg).is_err());
    ...
    fn test_can_invoke_edit_path_quota() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::EditPathQuota].into_iter().collect(),
            )
            .unwrap();
        assert!(can_invoke_message(&deps.as_mut(), &info_foobar, &msg).is_ok());
        assert!(can_invoke_message(&deps.as_mut(), &info_foobarbaz, &msg).is_err());
    ...
    fn test_can_invoke_remove_message() {
        ...
        RBAC_PERMISSIONS
            .save(
                &mut deps.storage,
                "foobar".to_string(),
                &vec![Roles::RemoveMessage].into_iter().collect(),
            )
            .unwrap();
        assert!(can_invoke_message(&deps.as_mut(), &info_foobar, &msg).is_ok());
        assert!(can_invoke_message(&deps.as_mut(), &info_foobarbaz, &msg).is_err());
    ...
    fn test_can_invoke_process_messages() {
        ...
        let msg = ExecuteMsg::ProcessMessages {
            count: Some(1),
            message_ids: None,
        };
        ...
        assert!(can_invoke_message(&deps.as_mut(), &info_foobar, &msg).is_ok());
        assert!(can_invoke_message(&deps.as_mut(), &info_foobarbaz, &msg).is_ok());

        // try again with message_ids Some

        let msg = ExecuteMsg::ProcessMessages {
            count: None,
            message_ids: Some(vec!["foobar".to_string()]),
        };
