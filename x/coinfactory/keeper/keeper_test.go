...
func (suite *KeeperTestSuite) Setup() {
	...
	CoinfactoryKeeper := suite.GetNeutronZoneApp(suite.ChainA).CoinfactoryKeeper
	err := CoinfactoryKeeper.SetParams(suite.ChainA.GetContext(), types.NewParams(
	...
	suite.msgServer = keeper.NewMsgServerImpl(*CoinfactoryKeeper)
