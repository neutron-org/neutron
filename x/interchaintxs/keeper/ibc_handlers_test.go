...
func TestHandleAcknowledgement(t *testing.T) {
	...
	cmKeeper.EXPECT().AddContractFailure(ctx, p, contractAddress.String(), "ack")
	...
	cmKeeper.EXPECT().AddContractFailure(lowGasCtx, p, contractAddress.String(), "ack").Do(func(ctx sdk.Context, channelId, address string, ackID uint64, ackType string) {
...
func TestHandleTimeout(t *testing.T) {
	...
	cmKeeper.EXPECT().AddContractFailure(ctx, p, contractAddress.String(), "timeout")
	feeKeeper.EXPECT().DistributeTimeoutFee(ctx, relayerAddress, feetypes.NewPacketID(p.SourcePort, p.SourceChannel, p.Sequence))
	err = icak.HandleTimeout(ctx, p, relayerAddress)
	require.NoError(t, err)
	require.Empty(t, store.Get(ShouldNotBeWrittenKey))
