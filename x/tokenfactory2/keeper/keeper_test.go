...
func (suite *KeeperTestSuite) Setup() {
	...
	TokenFactory2Keeper := suite.GetNeutronZoneApp(suite.ChainA).TokenFactory2Keeper
	err := TokenFactory2Keeper.SetParams(suite.ChainA.GetContext(), types.NewParams(
	...
	suite.msgServer = keeper.NewMsgServerImpl(*TokenFactory2Keeper)
...
func (suite *KeeperTestSuite) SetupTokenFactory() {
	suite.GetNeutronZoneApp(suite.ChainA).TokenFactory2Keeper.CreateModuleAccount(suite.ChainA.GetContext())
}
